...
REFERENCECP=""
INPUTFQ1=""
#Modified for single-end use - Simon Uribe-Convers
#INPUTFQ2=""
REFERENCEMT=""
...
echo
bowtie2 -x $REFERENCECP2 -U $INPUTFQ1 -S $BOWTIE2CP || {
  echo
...
if [ -n "$REFERENCEMT" ]; then
  ...
  echo
  bowtie2 -x $REFERENCEMT2 -U `echo $FASTQNOCP`.fq -S $BOWTIE2MT || {
    echo
...
echo "${CYAF}Converting FASTQ to FASTA. This may take longer time.${NORM}"
 #Modified for single-end use - Simon Uribe-Convers
if [ -n "$REFERENCEMT" ]; then
fastq_to_fasta -i $FASTQNOMT.fq -o $FLASHOUT.extendedFrags.fa || {
  echo
  ...
  exit 1
    ...
    fastq_to_fasta -i $FASTQNOCP.fq -o $FLASHOUT.extendedFrags.fa || {
     echo
     echo "${REDF}${BOLD}Error!${NORM} ${CYAF}Conversion of FASTQ to FASTA failed.${NORM} Aborting. Check if file"
     echo "${REDF}$FLASHOUT.extendedFrags.fastq${NORM} is correct."
     echo
